...
void AppMonitor::update_status() {
    ...
    const char *monitor_icon = (tracing_state_ == TRACING) ? "OK" : "NO";
    bool abi_ok = d_status.supported && d_status.daemon_running && d_status.zygote_injected;
    const char *abi_icon = abi_ok ? "OK" : "NO";
    ...
    std::string installed_status = runtime_status;
...
bool AppMonitor::prepare_environment() {
    ...
    auto is_generated_key = [](std::string_view line) {
        return line.starts_with("monitor_status=") || line.starts_with("monitor_stop_reason=") ||
               line.starts_with("zygote_") || line.starts_with("daemon_");
    };
    file_readline(false, orig_prop.get(), [&](std::string_view line_view) -> bool {
        ...
        if (line.starts_with("description=")) {
            if (post) return true;
            post = true;
            std::string desc_val = line.substr(12);
            bool cleaning = true;
            while (cleaning) {
                cleaning = false;
